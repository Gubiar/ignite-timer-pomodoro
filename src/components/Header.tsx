...
export default function Header() {
  ...
  function Relogio() {
    return (
      ...
      >
        <path
          className={`group-hover:fill-green-500 group-focus:fill-green-500 ${location.pathname === "/" ? "fill-green-500" : "fill-gray-300"}`}
          fillRule="evenodd"
  ...
  function List() {
    return (
      ...
      >
        <path
          className={`group-hover:fill-green-500 group-focus:fill-green-500 ${location.pathname === "/history" ? "fill-green-500" : "fill-gray-300"}`}
          fillRule="evenodd"
  ...
  return (
    <header className="flex h-14 w-full items-center justify-between px-8 py-14 max-sm:h-14 max-sm:py-12">
      <img src={Logo} alt="Logo" />
      <div className="flex gap-4">
        <button
          onClick={goToHome}
          className="group flex items-center justify-center rounded-lg p-4 transition-all duration-300 ease-in-out"
        >
          <Relogio />
        </button>
        <button
